...
# proj_path <- "/Volumes/HurlbertLab/"

## species range maps directory
range_dir <- paste0(proj_path, "bird_range_shps/All/")
# range_dir <- "/Volumes/HurlbertLab/GIS/birds/All/All/"

## Species
prob_spp <- c(4880, 7290, 7610, 5290, 5011, 4950, 5080)
...
# 
# routes <- read_csv(paste0(proj_path, "Databases/BBS/2017/bbs_routes_20170712.csv"))
# weather <- read_csv(paste0(proj_path, "Databases/BBS/2017/bbs_weather_20170712.csv"))
# counts <- read_csv(paste0(proj_path, "Databases/BBS/2017/bbs_counts_20170712.csv"))
...
concave_area <- function(aou, df) {
  ...
  concave_sub <- st_intersection(st_buffer(breeding_range, 0), concave)
  
  concave_nobr <- st_difference(concave, st_buffer(breeding_range, 0))
  
  concave_nobr_cast <- concave_nobr %>%
  ...
  # Fix invalid geoms for problem spp
  if(all(st_is_valid(concave_all))) {
    concave_all <- concave_all
  } else {
    concave_all <- st_make_valid(concave_all)
  }
  ...
  # concave_all_BbsBr <- st_union(st_make_valid(concave_all_noBbsBr), bbs_br)
  # 
  # range_outlier <- as.numeric(sum(st_area(st_make_valid(concave_all_noBbsBr))))
  # range_outlier_denom <- as.numeric(sum(st_area(st_make_valid(concave_all_noBbsBr))))
  
  range_outlier <- NA
  range_outlier_denom <- NA
...
range_samp <- foreach(i=1) %dopar% {
  ...
  spp_counts_t1 <- counts_subs %>%
    filter(year >= 1976, year < 1981, aou %in% prob_spp) %>%
    mutate(lat_cell = round(latitude),
           lon_cell = round(longitude),
           cell_id = paste0(lon_cell, ",", lat_cell)) %>%
  ...
  spp_counts_t2 <- counts_subs %>%
    filter(year >= 2013, year <= 2017, aou %in% prob_spp) %>%
    mutate(lat_cell = round(latitude),
           lon_cell = round(longitude),
           cell_id = paste0(lon_cell, ",", lat_cell)) %>%
    filter(stateroute %in% sample_routes$stateroute) %>%
...
write.csv(range_metrics, "/proj/hurlbertlab/gdicecco/avian-range-shifts/derived_data/range_metrics_sampled_lo_concav_fix_spp.csv", row.names = F)
